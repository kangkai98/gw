...
class OnlineCaptureManager:
    ...
    def start(
        self, interface: str, interval_sec: int = 60, bpf_filter: str = "tcp", idle_timeout_sec: int = 120
    ) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            self._status = CaptureStatus(
                ...
                started_at=now,
                message=f"在线监听已启动：{interface}，每 {interval_sec} 秒采集一次，空闲超时 {idle_timeout_sec} 秒",
            )
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if self._stop_event.is_set() and (not file_path.exists() or file_path.stat().st_size == 0):
                    break
                detected, inserted, ready_flows, analyzed_pcap = self._analyze_window(file_path, idle_timeout_sec)
                finished = _now_text()
                with self._lock:
                    ...
                    self._status.message = f"窗口分析完成：就绪流 {ready_flows} 个，检测 {detected} 条，入库 {inserted} 条"
    ...
    def _analyze_window(self, file_path: Path, idle_timeout_sec: int) -> tuple[int, int, int, Path | None]:
        ...
        try:
            ...
            return len(entries), inserted, len(ready_keys), analyzed_pcap
        finally:
    ...
    def _ready_flow_keys(self, observation_ts: float, idle_timeout_sec: int) -> list[str]:
        ready: list[str] = []
