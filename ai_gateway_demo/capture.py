...
from .db import insert_app_flow_stat, insert_entry, insert_traffic_summary, list_self_hosted
...
class OnlineCaptureManager:
    ...
    _app_flow_cache: dict[str, AppFlowStats] = field(default_factory=dict, init=False)
    ...
    def _run_loop(
        ...
        try:
            ...
            self._flush_all_app_flow_stats()
            self._proc = None
    ...
    def _remember_third_party_sni_from_packets(self, packets: list[CachedTcpPacket]) -> None:
        ...
        with self._lock:
            for packet in packets:
                ...
                if not payload:
                    continue
                sni = _extract_tls_sni_from_payload(payload)
    ...
    def _observe_app_traffic_packets(self, packets: list[CachedTcpPacket]) -> None:
        if not packets:
            return
        with self._lock:
            for packet in packets:
                tcp_meta = _extract_ipv4_tcp_meta_from_ethernet(packet.raw_packet)
                if tcp_meta is None:
                    continue
                src, sport, dst, dport, _ = tcp_meta
                cache_key = _normalize_flow_key(packet.flow_key)
                src_endpoint = f"{src}:{sport}"
                dst_endpoint = f"{dst}:{dport}"
                active = self._app_flow_cache.get(cache_key)
                if active is None:
                    payload = _extract_tcp_payload_from_ethernet(packet.raw_packet)
                    sni = _extract_tls_sni_from_payload(payload)
                    app_name = _third_party_minor_for_sni(sni)
                    if not app_name:
                        continue
                    active = AppFlowStats(
                        app_name=app_name,
                        ...
                        flow_key=cache_key,
                        client_endpoint=src_endpoint,
                        server_endpoint=dst_endpoint,
                        start_ts=packet.ts,
                        end_ts=packet.ts,
                    )
                    self._app_flow_cache[cache_key] = active

                active.start_ts = min(active.start_ts, packet.ts)
                active.end_ts = max(active.end_ts, packet.ts)
                if src_endpoint == active.client_endpoint:
                    active.uplink_bytes += packet.wire_len
                elif src_endpoint == active.server_endpoint:
                    active.downlink_bytes += packet.wire_len
                elif dst_endpoint == active.server_endpoint:
                    active.uplink_bytes += packet.wire_len
                else:
                    active.downlink_bytes += packet.wire_len
                bucket = int(packet.ts)
                active.buckets[bucket] = active.buckets.get(bucket, 0) + packet.wire_len
                if packet.tcp_flags & 0x05:
                    self._flush_app_flow_stat_locked(cache_key)

    def _flush_app_flow_stat_locked(self, cache_key: str) -> None:
        active = self._app_flow_cache.pop(cache_key, None)
        if active is None:
            return
        peak_bps = float(max(active.buckets.values(), default=0) * 8)
        insert_app_flow_stat(
            {
                "app_name": active.app_name,
                "sni": active.sni,
                "flow_key": active.flow_key,
                "protocol": "TCP",
                "client_endpoint": active.client_endpoint,
                "server_endpoint": active.server_endpoint,
                "start_time_real": _format_ts(active.start_ts),
                "end_time_real": _format_ts(active.end_ts),
                "duration_sec": max(0.0, active.end_ts - active.start_ts),
                "uplink_bytes": active.uplink_bytes,
                "downlink_bytes": active.downlink_bytes,
                "peak_bps": peak_bps,
            }
        )

    def _flush_all_app_flow_stats(self) -> None:
        with self._lock:
            keys = list(self._app_flow_cache.keys())
            for key in keys:
                self._flush_app_flow_stat_locked(key)
    ...
    def _analyze_window(
        ...
        if file_path.exists():
            if file_path.stat().st_size > 0:
                ...
                self._observe_app_traffic_packets(packets)
        ...
        with self._lock:
            for key in ready_keys:
                self._flush_app_flow_stat_locked(_normalize_flow_key(key))
        deleted_pcaps += _cleanup_expired_pcaps(
            self.output_dir,
            pcap_retention_sec,
            keep_latest_online=self._proc is not None and self._proc.poll() is None,
...
def _extract_tls_sni_from_payload(data: bytes) -> str | None:
    # Minimal TLS ClientHello SNI parser kept local to capture.py so application-level
    # traffic accounting does not depend on parser.py's AI request parser.
