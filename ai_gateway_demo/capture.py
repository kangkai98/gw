...
class OnlineCaptureManager:
    ...
    def _run_loop(self, interface: str, interval_sec: int, bpf_filter: str, idle_timeout_sec: int) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                ...
                detected, inserted, ready_flows, ready_packets = self._ingest_and_process_window(
                    file_path,
                    idle_timeout_sec,
                ...
                with self._lock:
                    ...
                    self._status.last_finalized_flows = ready_flows
                    self._status.last_finalized_packets = ready_packets
                    self._status.total_windows += 1
                    self._status.total_detected += detected
                    self._status.total_inserted += inserted
                    self._status.total_finalized_flows += ready_flows
                    self._status.total_finalized_packets += ready_packets
                    self._status.current_file = None
                    self._sync_cache_counts_locked()
                    self._status.message = (
                        f"窗口处理完成：本次处理 {ready_flows} 条流/{ready_packets} 个报文，"
                        f"检测 {detected} 条，入库 {inserted} 条；缓存 {self._status.cached_flows} 条流"
                    )
    ...
    def _ingest_and_process_window(
        ...
        return self._process_ready_flows(idle_timeout_sec=idle_timeout_sec, flush_all=flush_all)
    ...
    def _process_ready_flows(self, idle_timeout_sec: int, flush_all: bool = False) -> tuple[int, int, int, int]:
        now = time.time()
        with self._lock:
            ready_keys = [
                key
            ...
            for key in ready_keys:
                ready_packets.extend(self._flows[key].packets)
        ...
        with self._lock:
            ...
            self._sync_cache_counts_locked()
        return detected, inserted, len(ready_keys), len(ready_packets)
