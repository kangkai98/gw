...
ENTRY_COLUMNS: dict[str, str] = {
    ...
    "ai_up_bytes": "INTEGER NOT NULL DEFAULT 0",
    "total_up_bytes": "INTEGER NOT NULL DEFAULT 0",
    "ai_down_bytes": "INTEGER NOT NULL DEFAULT 0",
    ...
    try:
        conn.execute(
            ...
            INSERT INTO entries (
                ...
                input_tokens, output_tokens, ai_up_bytes, total_up_bytes, ai_down_bytes, total_down_bytes
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                entry["input_tokens"],
                entry["output_tokens"],
                entry.get("ai_up_bytes", 0),
                entry.get("total_up_bytes", 0),
                entry.get("ai_down_bytes", 0),
                entry.get("total_down_bytes", 0),
            ),
        )
        conn.commit()
        return True
    ...
    try:
        ...
        totals = conn.execute(
            ...
            SELECT
                ...
                COALESCE(SUM(ai_up_bytes), 0) AS total_ai_up_bytes,
                COALESCE(SUM(total_up_bytes), 0) AS total_up_bytes,
                COALESCE(SUM(ai_down_bytes), 0) AS total_ai_down_bytes,
                COALESCE(SUM(total_down_bytes), 0) AS total_down_bytes,
        ...
        return {
            ...
            "total_ai_up_bytes": totals["total_ai_up_bytes"],
            "total_up_bytes": totals["total_up_bytes"],
            "total_ai_down_bytes": totals["total_ai_down_bytes"],
            "total_down_bytes": totals["total_down_bytes"],
            "ai_up_ratio": round((totals["total_ai_up_bytes"] / totals["total_up_bytes"] * 100), 1) if totals["total_up_bytes"] else 0,
            "ai_down_ratio": round((totals["total_ai_down_bytes"] / totals["total_down_bytes"] * 100), 1) if totals["total_down_bytes"] else 0,
            "rps": round(rps, 1),
