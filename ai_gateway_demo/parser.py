...
def _build_entry(
    ...
    total_up_bytes = sum(len(p.raw or b"") for p in ordered if p.src == client_ip)
    total_down_bytes = sum(len(p.raw or b"") for p in ordered if p.src == server_ip)
    ai_up_bytes = sum(len(p.raw or b"") for p in up_app)
    ai_down_bytes = sum(len(p.raw or b"") for p in down_after_start)
    ...
    return {
        ...
        "ai_up_bytes": ai_up_bytes,
        "total_up_bytes": total_up_bytes,
        "ai_down_bytes": ai_down_bytes,
