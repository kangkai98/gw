...
PARSER_CONFIG_PATH = Path("config/parser_config.json")


def _load_parser_config() -> dict:
    try:
        with PARSER_CONFIG_PATH.open("r", encoding="utf-8") as f:
            data = json.load(f)
...
_PARSER_CONFIG = _load_parser_config()


TOKEN_LOG_ENABLE = bool(_PARSER_CONFIG.get("TOKEN_LOG_ENABLE", True))
TOKEN_LOG_PATH = str(_PARSER_CONFIG.get("TOKEN_LOG_PATH", os.path.join("uploads", "token_calc.log")))
...
MIN_GO_RUN = int(_PARSER_CONFIG.get("MIN_GO_RUN", 10))
MIN_ROUNDTRIPS = int(_PARSER_CONFIG.get("MIN_ROUNDTRIPS", 30))
RETURN_SUM_MAX = int(_PARSER_CONFIG.get("RETURN_SUM_MAX", 1400))
ALLOW_BAD_RET_BURSTS = int(_PARSER_CONFIG.get("ALLOW_BAD_RET_BURSTS", 2))
SEGMENT_MERGE_GAP_SEC = float(_PARSER_CONFIG.get("SEGMENT_MERGE_GAP_SEC", 2.0))
GO_LEN_MAX = int(_PARSER_CONFIG.get("GO_LEN_MAX", 200))
