...
function CapturePanel({ capture, form, setForm, onStart, onStop, busy }) {
  ...
  return html`
    <div className="rounded-[26px] border border-white/8 bg-white/[0.04] p-5">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_110px_130px]">
      ...
      <div className="mt-4 rounded-[20px] border border-white/8 bg-black/20 p-4 text-xs leading-6 text-slate-400">
        ...
        <div>周期：${capture?.interval_sec || form.interval_sec || 60}s · idle：${capture?.idle_timeout_sec || form.idle_timeout_sec || 120}s · 最近窗口：${capture?.last_window_finished_at || '--'}</div>
...
function App() {
  ...
  const [capture, setCapture] = useState({ running: false, interval_sec: 60, idle_timeout_sec: 120, message: '未启动在线监听' });
  const [captureForm, setCaptureForm] = useState({ interface: '', interval_sec: '60', idle_timeout_sec: '120', bpf_filter: 'tcp' });
