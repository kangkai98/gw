  ...
  return html`
    <div className="rounded-[26px] border border-white/8 bg-white/[0.04] p-5">
      ...
      <div className="mt-4 rounded-[20px] border border-white/8 bg-black/20 p-4 text-xs leading-6 text-slate-400">
        ...
        <div>周期：${capture?.interval_sec || form.interval_sec || 15}s · idle：${capture?.idle_timeout_sec || form.idle_timeout_sec || 15}s · 最长缓存：${capture?.max_flow_duration_sec ?? form.max_flow_duration_sec ?? 120}s</div>
...
function App() {
  ...
  const [capture, setCapture] = useState({ running: false, interval_sec: 15, idle_timeout_sec: 15, max_flow_duration_sec: 120, pcap_retention_sec: 0, message: '未启动在线监听' });
  const [captureForm, setCaptureForm] = useState({ interface: 'WLAN', interval_sec: '15', idle_timeout_sec: '15', max_flow_duration_sec: '120', pcap_retention_sec: '0', bpf_filter: 'tcp' });
  ...
  const onStartCapture = async () => {
    ...
    try {
      const result = await postForm('/api/capture/start', {
        ...
        max_flow_duration_sec: captureForm.max_flow_duration_sec || '120',
