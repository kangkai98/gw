...
- **多流识别**：先对每条双向 TCP 流做 AI 相关性评分（SNI/host 关键词、上下行语义特征、流量规模、自建IP命中），按分数阈值一次选出多条 AI 流，而不是只取单条。
- **一问一答切分**：使用状态机切分 turn（出现“已响应后新的上行请求包”或“长空闲后新的上行请求包”即开启新条目），不再仅依赖固定时间阈值。
- **质量过滤**：条目必须满足 `latency>0`、`ttft>0`、`ttft>=ttfb`、输入输出 token 均大于 0，避免误识别脏数据入库。
